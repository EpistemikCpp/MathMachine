 ...
 * -----------------------------------------------
 *  $File: //depot/CppProj/VS6/mathMachine/mathMachineDlg.cpp $
 *  $Revision: #3 $
 *  $Change: 68 $
...
// in case the user enters a value directly into the Op edit box
  ...
  if( m_boolInSession )
	{
		...
		{	// start a new session
		  m_boolInSession = true ;
			...
			{
				// session range will be from 'number' to 'number + range value' 
				...
				{
					// session range will be from 0 to the chosen range value
  ...
  if( m_boolInSession )
	{
		...
		bool nextRhv = processResponse();// return indicates if response was correct
    
		// see if we are finished the current session
		if( m_nResponseCount < m_nNumQuestions )
			// repeat current question if response was wrong, i.e. nextRhv == False
		  displayQuestion( nextRhv );
		else
			{
				endSession();
...
void CMathMachineDlg::displayQuestion( bool newRhv )
{
	...
	{
		// get a random value in the current valid range
...
bool CMathMachineDlg::processResponse()
{
	...
	if( !responseOk )
	{
		...
		// only count one error for the current question
		if( !m_boolErrorState )
		{
			m_nErrorCount++ ;
	...
	return responseOk ;// repeat the current question if response is incorrect
...
void CMathMachineDlg::getAndValidateData()
{
	// transfer the data from the gui to the vars and check the values
	if( UpdateData() == FALSE )
	{
		// i.e. at least one of the gui values was improper
	    ...
	    if( m_nNumQuestions < MIN_NUM_QUESTIONS  ||  m_nNumQuestions > MAX_NUM_QUESTIONS )
	      m_nNumQuestions = DEFAULT_NUM_QUESTIONS ;
			...
			// NO FURTHER NEED FOR THIS - NOW WILL REPEAT RANGE NUMBERS IF NECESSARY
	    // make sure the range is larger than the number of questions
	...
	// update the gui with the final values
	UpdateData( FALSE );
