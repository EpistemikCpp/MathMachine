...
void CMathMachineDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MinMaxInt(pDX, m_nNumQuestions, 1, 99);
	DDX_Text(pDX, IDC_RIGHT_RANGE, m_nRightRange);
	DDV_MinMaxInt(pDX, m_nRightRange, 12, 99);
	//}}AFX_DATA_MAP
}
  ...
  if( m_boolInSession )
	{
		...
		if( m_nResponseCount < NUM_QUESTIONS )//(DEFAULT_RANGE+1) )
...
void CMathMachineDlg::endSession()
{
	...
	CString timetaken = timemsg + min + " minutes & " + sec + " seconds.\n\n" ;
	// error count
	char g[MAX_NUM_DIGITS] = {0} , t[MAX_NUM_DIGITS] = {0} ;
	sprintf( g, "%d", NUM_QUESTIONS/*(DEFAULT_RANGE+1)*/ - m_nErrorCount );
	CString good = g ;
	sprintf( t, "%d", NUM_QUESTIONS/*DEFAULT_RANGE+1*/ );
	CString tot = t ;
	CString score = "You got " + good + " right out of " + tot + "!\n\n" ;
	MessageBox( score + timetaken + m_strMesg,
				m_strCaption, MB_OK | MB_ICONEXCLAMATION );
...
{
	// Move reporting code here and call this function from endSession()
